...
int main(void) {
    ...
    // Das neue vertices-Array: Jede Zeile enthält nun X, Y, Z (Position) und R, G, B (Farbe)
    float vertices[] = {
        // Positionen          // Farben (Rot, Grün, Blau)
    ...
    // NEU: Die Schrittweite (Stride) von einer Zeile zur nächsten beträgt nun 6 * float
    int stride = 6 * sizeof(float);

    // 1. Attribut: Position (Location 0)
    // Liest 3 Floats, startet bei Offset 0
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, stride, (void*)0);
    glEnableVertexAttribArray(0);

    // 2. Attribut: Farbe (Location 1) - NEU!
    // Liest 3 Floats, startet nach den ersten 3 Positions-Floats (Offset: 3 * sizeof(float))
